...
def login_page():
        ...
        if st.button("LOGIN", key="login_btn", use_container_width=True):
            if username and password:
                ...
                if user_id:
                    ...
                    st.error("❌ INVALID LAND YE TO GALT H BDK!")
            else:
    ...
    with tab2:
        ...
        confirm_password = st.text_input("CONFIRM RANDI PASSWORD", key="confirm_password", type="password", placeholder="Re-enter your password")
        
        if st.button("CREATE BDK ACCOUNT", key="signup_btn", use_container_width=True):
            if new_username and new_password and confirm_password:
                if new_password == confirm_password:
                    success, message = db.create_user(new_username, new_password)
                    if success:
                        st.success(f"✅ {message} PLEASE BDK LOGIN NOW!")
                    else:
                        st.error(f"❌ {message}")
                else:
...
def main_app():
    ...
    if user_config:
        ...
        with tab1:
            st.markdown('<div class="section-title">END TO END SETTING CHUDAI</div>', unsafe_allow_html=True)
        ...
        with tab2:
            st.markdown('<div class="section-title">🚀 AUTOMATION LAND CONTROL</div>', unsafe_allow_html=True)
            ...
            with col1:
                ...
                st.metric("MESSAGES GAY CHUT ME KO SENT", st.session_state.automation_state.message_count)
