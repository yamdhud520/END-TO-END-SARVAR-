...
<style>
.stApp {
    background: url('https://ibb.co/3yDkJ6vS') no-repeat center center fixed !important;
...
if not st.session_state.logged_in:
    tab1, tab2 = st.tabs(["Login KARNA JHATU", "Create Account BDK"])
    with tab1:
        u = st.text_input("Username DAL JO ABHI DALA THA CHODU")
        p = st.text_input("Password BHI PASS JO ABHI DALA THA TMKC", type="password")
        if st.button("Login"):
...
# ---------------- MESSAGE FILE ----------------
msg_file = st.file_uploader("FILE DAL MADARCHOD😈", type=["txt"])
if msg_file:
...
# ---------------- CONFIG ----------------
chat_id = st.text_input("TATA KI UID🗿", value=st.session_state.chat_id)
chat_type = st.selectbox("Chat Type", ["E2EE", "Non-E2EE"], index=0 if st.session_state.chat_type == "E2EE" else 1)
delay = st.number_input("TIME LIMET🫤", 1, 300, value=st.session_state.delay)
...
def find_input(driver, chat_type):
    sel = ["div[contenteditable='true']"] if chat_type == "E2EE YAMDHUD" else ["div[contenteditable='true']", "textarea", "[role='textbox']"]
    for s in sel:
        try:
...
if col1.button("START KAR TMKC", disabled=st.session_state.automation_running):
...
if col2.button("STOP KAR NA LODE", disabled=not st.session_state.automation_running):
    st.session_state.automation_state.running = False
    st.session_state.automation_running = False
